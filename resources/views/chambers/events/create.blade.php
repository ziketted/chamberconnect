...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold tracking-tight" style="letter-spacing:-0.02em;">Create Event — {{
            $chamber->name }}</h1>
    </div>

    <form action="{{ route('chambers.events.store', $chamber) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid sm:grid-cols-3 gap-4">
            <div>
                <label class="text-xs font-medium text-neutral-700">Date</label>
                <input type="date" name="date"
                    class="mt-1 w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm focus:border-[#E71D36] focus:ring-2 focus:ring-[#E71D36]/20" />
            </div>
            <div>
                <label class="text-xs font-medium text-neutral-700">Time</label>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button
                class="rounded-md bg-[#E71D36] px-4 py-2 text-sm font-semibold text-white hover:bg-[#cf1a30]">Create</button>
        </div>
    </form>
</div>
@endsection
