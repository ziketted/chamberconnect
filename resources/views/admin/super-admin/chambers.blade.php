...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Gestion des Chambres</h1>
            <p class="mt-2 text-gray-600">Cliquez sur une chambre pour gérer ses membres et paramètres</p>
        </div>
        <a href="{{ route('chambers.create') }}" 
           class="bg-[#E71D36] text-white px-4 py-2 rounded-lg hover:bg-[#cf1a30] transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 truncate">Total Chambres</dt>
                        <dd class="text-lg font-medium text-gray-900">{{ $chambers->total() }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        <dt class="text-sm font-medium text-gray-500 truncate">Certifiées</dt>
                        <dd class="text-lg font-medium text-gray-900">{{ $chambers->where('verified', true)->count() }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900">{{ $chambers->where('verified', false)->count() }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900">{{ $chambers->sum(function($chamber) { return $chamber->members->count(); }) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 cursor-pointer chamber-card" 
            ...
            <div class="relative">
                @if($chamber->cover_image_path)
                    <img class="w-full h-32 object-cover rounded-t-lg" src="{{ $chamber->cover_image_path }}" alt="{{ $chamber->name }}">
                @else
                    <div class="w-full h-32 bg-gradient-to-r from-blue-500 to-purple-600 rounded-t-lg"></div>
                @endif
                ...
                <div class="absolute -bottom-6 left-6">
                    @if($chamber->logo_path)
                        <img class="h-12 w-12 rounded-full border-4 border-white shadow-lg" src="{{ $chamber->logo_path }}" alt="{{ $chamber->name }}">
                    @else
                        <div class="h-12 w-12 rounded-full border-4 border-white shadow-lg bg-gray-300 flex items-center justify-center">
            ...
            <div class="pt-8 pb-6 px-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600">{{ $chamber->members->where('pivot.role', 'manager')->count() }}</div>
                        <div class="text-xs text-gray-500">Gestionnaires</div>
                    </div>
                </div>
        ...
        <div class="col-span-full">
            <div class="text-center py-12">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">Aucune chambre</h3>
                <p class="mt-1 text-sm text-gray-500">Commencez par créer une nouvelle chambre de commerce.</p>
                <div class="mt-6">
                    <a href="{{ route('chambers.create') }}" 
...
<div id="assignManagerModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" onclick="closeAssignManagerModal()"></div>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="mt-1 relative">
                                    ...
                                    <div id="search_results" class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm hidden">
                                ...
                                <div id="selected_user" class="mt-3 hidden">
                                    <div class="flex items-center p-3 bg-purple-50 rounded-lg">
                                        ...
                                        <button type="button" onclick="clearSelectedUser()" class="ml-auto text-purple-600 hover:text-purple-800">
